...
class BGPTargetExtractor:
    ...
    def generate_combined_priority(self):
        ...
        # Sort by frequency (most seen = most important)
        priority_targets.sort(key=lambda x: x['frequency'], reverse=True)
        
        filename = 'targets_priority_combined.txt'
        with open(filename, 'w') as f:
            f.write("# Priority Traceroute Targets - All Taiwan ASNs\n")
            f.write("# BGP next-hop IPs (border/edge routers) sorted by frequency\n")
            f.write("# High frequency = used by many prefixes = likely core router\n\n")
            
            for target in priority_targets[:100]:
                asn_name = f"AS{target['asn']}"
                f.write(f"{target['ip']:15s}  {asn_name:8s}  freq:{target['frequency']:4d}  # {TAIWAN_ASNS[target['asn']]}\n")
        
        print(f"   ✓ {filename} (top 100 targets)")
